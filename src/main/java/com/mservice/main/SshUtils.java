...
public final class SshUtils {
    ...
    public static String execTail(String connectUri, String command) {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try {
            shellHoldConnect(connectUri, command, baos);
            return baos.toString();
        } catch (RuntimeException e) {
            log.warn(baos.toString());
            throw e;
    ...
    public static String shellHoldConnect(String username, String password, String host, String command) {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try {
            shellHoldConnect(username, password, host, command, baos);
            return baos.toString();
        } catch (RuntimeException e) {
            log.warn(baos.toString());
            throw e;
    ...
    public static String shellHasUser(String username, String password, String host, String command) {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try {
            shellHasUser(username, password, host, command, baos);
            return baos.toString();
        } catch (RuntimeException e) {
            log.warn(baos.toString());
            throw e;
    ...
    public static void shellHasUser(String username, String password, String host, String command, OutputStream out) {
        try{
            SessionHolder<ChannelShell> session = new SessionHolder<>("exec", username, password, host);
            log.info("Start execute with script: "+command);
    ...
    public static void shellHoldConnect(String username, String password, String host, String script, OutputStream out) {
        try{
            SessionHolder<ChannelShell> session = new SessionHolder<>("exec", username, password, host);
            log.info("Start execute with script: "+script);
            Channel channel= session.getChannel();
    ...
    public static void shellHoldConnect(String uri, String script, OutputStream out) {
        try{
            SessionHolder<ChannelShell> session = new SessionHolder<>("exec", URI.create(uri));
            log.info("Start execute with script: "+script);
            Channel channel= session.getChannel();
            ((ChannelExec)channel).setCommand(script);
            channel.setInputStream(null);
    ...
    private static void shellHasUser(SessionHolder<ChannelShell> session, InputStream is, OutputStream os) {
        try {
            ChannelShell channel = session.getChannel();
            channel.setInputStream(is, true);
            channel.setOutputStream(os, true);
            ...
            log.info("Starting shell for " + session.getWorkName());
            session.execute();
            session.assertExitStatus("Check shell output for error details.");
        } catch (InterruptedException | JSchException e) {
            throw new RuntimeException("Cannot execute script", e);
    ...
    public static class SessionHolder<C extends Channel> implements Closeable {
        ...
        public SessionHolder(String channelType, String username, String password, String host, Map<String, String> props) {
            this.channelType = channelType;
            this.username = username;
            this.password = password;
            this.host = host;
            this.session = newSessionUsername(props);
            this.channel = newChannel(session);
        }

        public SessionHolder(String channelType, String username, String password, String host) {
            this(channelType, username, password, host, ImmutableMap.of("StrictHostKeyChecking", "no"));
        }
        ...
        private Session newSessionUsername(Map<String, String> props) {
            try {
                Properties config = new Properties();
                config.putAll(props);

                JSch jsch = new JSch();
                Session newSession = jsch.getSession(getUsername(), getHost(), 22);
                newSession.setPassword(getPassword());
                newSession.setUserInfo(new User(getUsername(), getPassword()));
                newSession.setDaemonThread(true);
                newSession.setConfig(config);
                newSession.connect(DEFAULT_CONNECT_TIMEOUT);
                return newSession;
        ...
        public String getWorkName() {
            return "ssh://"+getUsername()+":"+getPassword()+"@"+getHost();
        }
    }
